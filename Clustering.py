...
class Clustering(object):
    def __init__(self, max_len=1000, dist_type='cos'):
        self.eps = 0.005
    ...
    def collect_target_samples(self, model, dataset):
        data_feat, data_gt, data_paths = [], [], []
        transform_signal = dataset.transform
        for example in dataset.imgs:
            path = example[0]
            ...
            data_paths += [path]
            data_gt += [to_cuda(torch.tensor([example[1]]))]
            data_feat += [feature_share.data]
    ...
    def feature_clustering(self, net, loader):
        ...
        self.collect_target_samples(net, loader)
        ...
        while True:
            ...
            # centers_1 = mask * centers + (1 - mask) * self.init_centers
            # equal = torch.equal(centers,centers_1)
